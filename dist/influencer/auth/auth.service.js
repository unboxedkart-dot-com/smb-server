...
let AuthService = class AuthService {
    constructor(influencerModel, refreshTokenModel, jwtService) {
    ...
    async setStatus() {
        await this.influencerModel.updateMany({
            $set: {
                isDeactivated: false,
                isDeleted: false,
            },
    ...
    async deleteAccount(userId) {
        ...
        const newUser = await this.influencerModel.findByIdAndUpdate(userId, {
            isDeleted: true,
    ...
    async loginUser(entireBody) {
        ...
        if (otpStatus ||
            ...
            const user = await this.influencerModel
                .findOne({
                ...
                .exec();
            if (user) {
                await this.influencerModel.findOneAndUpdate({ phoneNumber: { $eq: entireBody.phoneNumber } }, { lastLoggedIn: Date.now() });
    ...
    async createUser(entireBody) {
        const otpStatus = await this.verifyOtp(entireBody.phoneNumber, entireBody.otp);
        if (otpStatus) {
            const userDoc = await this.influencerModel.findOne({
                phoneNumber: {
            ...
            if (userDoc) {
                ...
                const newUser = new this.influencerModel({
                    phoneNumber: entireBody.phoneNumber,
                    name: entireBody.name,
                    emailId: entireBody.emailId,
                    contactsCount: entireBody.contactsCount,
                    profilePicUrl: entireBody.profilePicUrl,
                    state: entireBody.state,
                    district: entireBody.district,
...
AuthService = __decorate([
    __param(0, (0, mongoose_1.InjectModel)('Influencer')),
